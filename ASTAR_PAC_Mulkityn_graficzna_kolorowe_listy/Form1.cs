...
namespace PacmanAstar
{
    public partial class Form1 : Form
    {
        ...
        static async Task<List<(int, int)>> ZnajdzTrase((int, int) start, (int, int) cel, Form1 form)
        {
            ...
            while (otwarte.Count > 0)
            {
                ...
                form.KolorujWęzeł(obecny, Color.Red, false); 
                ...
                foreach ((int, int) sasiad in Sasiedzi(obecny))
                {
                    ...
                    {
                        otwarte.Add(sasiad);
                        form.KolorujWęzeł(sasiad, Color.Lime, true); 
                    }
        ...
        {
            if (!punktStartowy.HasValue || !punktCelowy.HasValue)
            {
                MessageBox.Show("Proszę wybrać zarówno punkt startowy, jak i celowy.", "Błąd", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            
            foreach (var punkt in trasa)
            {
            ...
            trasa = await ZnajdzTrase(punktStartowy.Value, punktCelowy.Value, this); 

            if (trasa.Count == 0)
            {
                MessageBox.Show("Nie znaleziono trasy.", "Informacja", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
        ...
        public void KolorujWęzeł((int, int) punkt, Color kolor, bool czyOtwarte)
        {
            int alpha = czyOtwarte ? 255 : 230; 
            using (Graphics g = this.CreateGraphics())
            {
                Rectangle rect = new Rectangle(
                    punkt.Item2 * TILE_SIZE,
                    punkt.Item1 * TILE_SIZE,
                    TILE_SIZE,
                    TILE_SIZE
                );

                using (Pen pen = new Pen(Color.FromArgb(alpha, kolor), 3))
                {
